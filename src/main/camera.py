...
class Camera:
    """
    Repersents of the size and position
    of the camera in relation to the world.
    """
    ...
    MAX_ZOOM_FACTOR = 3
    MIN_ZOOM_FACTOR = .2
    ZOOM_CHANGE_VALUE = .2  # don't change
    ...
    def _zoom(self, zoom_change: float) -> None:
        screen_center = (self.rect.width // 2, self.rect.height // 2)
        ...
        self.rect.width = self.rect.width / self.zoom
        self.rect.height = self.rect.height / self.zoom

    def zoom_in(self) -> None:
        self._zoom(self.ZOOM_CHANGE_VALUE)
    ...
    def move_center(self, pos: tuple[int, int]) -> None:
    ...
    def world_to_relative(self, world_pos: tuple[int, int]) -> tuple[int, int]:
        """Convert world coordinates to relative camera coordinates."""
    ...
    def relative_to_world(self, relative_pos: tuple[int, int]) -> tuple[int, int]:
        """Convert relative camera coordinates to world coordinates."""
        return round(self.rect.x + relative_pos[0] / self.zoom), round(self.rect.y + relative_pos[1] / self.zoom)

    def move_relative_to_zoom(self, x_change: int, y_change: int) -> None:
    ...
    def move_within_barriers(self, x_change: int, y_change: int) -> None:
        ...
        if x_change != 0:
            self.move_relative_to_zoom(x_change, 0)
        ...
        if y_change != 0:
            self.move_relative_to_zoom(0, y_change)
            for rect in self.barrier_rects:
                if rect.colliderect(self.rect):
                    self.move_relative_to_zoom(0, -y_change)
                    break
